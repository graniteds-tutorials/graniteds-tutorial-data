...
The server application is a simple CDI service which implements basic CRUD operations on user accounts
in an in-memory HSQL database. The updates on the persistent entities are dispatched to other clients though a
messaging destination.
The CDI service is actually an EJB to simplify transactions and persistence management but this not a requirement.
...
The service is a simple CDI bean which implements the following interface:
...
The service implementation is an EJB so we get transactions and persistence management for free.
