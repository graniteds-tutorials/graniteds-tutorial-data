...
data and display them on stock exchange-style blocks.
...
<2> Configure a +ServerSession+ object in the context with the server webSite and context path.
...
<5> Create a consumer for the topic named +dataTopic+ (see the server definiton <<server.destination,here>>)
...
.main/java/org/graniteds/tutorial/data/client/dataClient.java
----
include::{srcdir}/main/java/org/graniteds/tutorial/data/client/dataClient.java[tags=client-consume]
----
